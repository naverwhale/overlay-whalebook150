...
# Some categories don't need bashrc hooks, so don't spend time loading them.
cros_category_uses_bashrc_hooks() {
	case "${CATEGORY}" in
	acct-group|\
	acct-user|\
	virtual)
...
cros_stack_bashrc() {
	...
	# Make sure we only run the bashrc once.
	if [[ ! -v CROS_BASHRC_LOADED ]]; then
		# We use a directory other than $FILESDIR so that any CrOS specific
		# patches are not wiped out when we run `cros_portage_upgrade`.
		cfgd="$(dirname "${EBUILD}")/cros"
		cfg="${cfgd}/bashrc"
		if [[ -f "${cfg}" ]]; then
			if ! cros_category_uses_bashrc_hooks; then
				die "packages in category ${CATEGORY} may not use bashrc overrides"
			fi

			CROS_SKIP_LEGACY_BASHRC=1
			CROS_FILES_DIR="${cfgd}"
			# shellcheck disable=SC1090
			source "${cfg}"
		fi
		CROS_BASHRC_LOADED=1
	fi

	# TODO(b/246902767): Remove the legacy mechanism.
	if [[ ! -v CROS_SKIP_LEGACY_BASHRC ]]; then
...
cros_stack_hooks() {
	...
	for func in $(compgen -A function "cros_${phase}_") ; do
		if ${header} ; then
...
# These categories should never need bashrc hooks.
if cros_category_uses_bashrc_hooks; then
	cros_setup_hooks
fi
...
# Since we're storing the wrappers in a board sysroot, make sure that is
# actually in our PATH.  The wrappers rely on tools in the SDK, so it goes
# at the end.
#
# The broot comes first so conflicts with the SDK prefer the per-board broot.
cros_pre_pkg_setup_sysroot_build_bin_dir() {
	PATH="${CROS_BUILD_BOARD_TREE}/broot/usr/bin:${PATH}:${CROS_BUILD_BOARD_BIN}"
	BUILD_PKG_CONFIG_PATH="${CROS_BUILD_BOARD_TREE}/broot/usr/share/pkgconfig"
}
...
cros_pre_src_prepare_build_toolchain_catch() {
	...
	_sdk_build_filter_pkg_config() {
		...
		*/gdb:*) return 1;;
		...
		# TODO(b/314023895): Renode invokes `mkbundle` from dev-lang/mono which
		# has hardcoded shell pipelines invoking `pkg-config`.
		app-emulation/renode:*) return 1;;
	...
	_sdk_build_filter_ld() {
		...
		cross-*/gcc:*|\
		cross-*/go:*|\
		cross-embedded-*/coreboot-sdk:*|\
		cross-embedded-*/coreboot-sdk-picolibc:*|\
		cross-embedded-*/coreboot-sdk-libstdcxx:*|\
...
EOF
	...
	# Temporarily allow haskell packages.
...
skip_coverage() {
	# Note: We can only check "use coverage" if coverage is in IUSE.
	# There is a function in_iuse() which checks for that, but it's
	# not always defined at this point, so we do the check manually.
	local iuse_arr=( ${IUSE} )
	local iuse=("${iuse_arr[@]#[+-]}")
	local flag
	for flag in gcov coverage rust-coverage; do
...
cros_post_src_install_coverage_logs() {
	# Exit early if we don't have coverage flags set.
	# This avoids doing the work of looking for files with coverage
	# information when we don't have any, and also avoids failures
	# on packages which contain test data that we would otherwise
	# try and fail to process (e.g. sys-devel/llvm).
	if skip_coverage; then
		einfo "Skipping coverage calculation as coverage flags not defined."
		return
	fi
	...
	local all_objects
	readarray -t all_objects < <(scanelf -qRy -k__llvm_covmap \
			-F'#k%F' "${OUT}" "${WORKDIR}" \
			"${CARGO_TARGET_DIR}/ecargo-test/${CHOST}")

	local objects
	local external_llvm_cov="external/llvm-raw/llvm/test/tools/llvm-cov"
	local element
	for element in "${all_objects[@]}"; do
		# Update array to remove any external llvm-cov object files.
		if [[ "${element}" != *"${external_llvm_cov}"* ]]; then
			objects+=("${element}")
		fi
	done

	if [[ "${#objects[@]}" -eq 0 ]]; then
		einfo "No object files found with LLVM instrumented data."
		return
	fi
	...
	if [ ! -d "${coverage_path}" ] || [ -z "$(ls -A "${coverage_path}")" ]; then
		...
		local chromite_dir="${CHROMITE_DIR:-/mnt/host/source/chromite}"
		"${chromite_dir}/scripts/cros_generate_coverage_artifacts" \
			"${objects[@]/#/--object-files=}" --output-dir "${hpt_cov_dir}" || die
		einfo "E2E coverage report for ${PN} generated at ${hpt_cov_dir}"
		return
	fi

	# TODO(b/305977232): Move all this computation to the coverage script.
	local report_path
	if [[ $(cros_target) == "cros_host" ]]; then
		...
		llvm-cov export "${objects[@]/#/--object=}" \
		...
		llvm-cov show "${objects[@]/#/--object=}" -format=html \
			-instr-profile="${cov_dir}/${PN}.profdata" \
			-output-dir="${cov_dir}" || die
		# Make coverage data readable for all users.
		chmod -R a+rX "${cov_dir}" || die "Could not make ${cov_dir} readable"
		report_path="${EXTERNAL_TRUNK_PATH}/out${SYSROOT}/${rel_cov_dir}/index.html"
	fi
	elog "Coverage report for ${PN} generated at file://${report_path}"
}

# Configure portage to allow chromite's local build telemetry to function.
cros_pre_src_configure_configure_chromite_local_telemetry() {
	addwrite /var/log/telemetry
	addwrite /var/log/portage/telemetry
	addwrite /home/"${PORTAGE_USERNAME}"/.config/chromite/
}
...
cros_log_failed_packages() {
	echo "${CATEGORY}/${PF} ${EBUILD_PHASE:-"unknown"}" >> "/tmp/FAILED_PACKAGES"
...
# This hook enables b/403315166 to work. Please see that bug for the design doc and motivation
# pertaining to this.
cros_pre_src_configure_add_pkg_to_cflags() {
	local pkg_flag="CROSTC_ADD_IMPLICIT_CFLAGS_FOR=${CATEGORY}/${PN}"
	export "${pkg_flag}"
	export CPPFLAGS+=" -D_${pkg_flag} "
	export BUILD_CPPFLAGS+=" -D_${pkg_flag} "
}
...
filter_unsupported_gcc_flags() {
	...
	for var in CFLAGS CXXFLAGS LDFLAGS; do
		for flag in ${!var}; do
			case "${flag}" in
			"-Xcompiler" | "-Wl,--icf=all" | "--unwindlib=libunwind" )
				# Ignore these entirely
				continue
				;;

			"-march=meteorlake" )
				# meteorlake support was added with GCC 13 (currently using GCC 10.2.0)
				flag="-march=skylake"
				;;

			-march=x86-64-v* )
				# x86_64 Microarchitecture Levels support was added with GCC 11
				flag="-march=x86-64"
				;;

			"-march=znver3" )
				# znver3 support was added with GCC 11 (currently using GCC 10.2.0)
				flag="-march=znver2"
				;;
			esac
			flags+=("${flag}")

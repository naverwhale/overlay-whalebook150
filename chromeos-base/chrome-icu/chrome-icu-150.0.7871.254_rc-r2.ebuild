...
inherit binutils-funcs chromium-source cros-constants cros-sanitizers flag-o-matic multilib cros-toolchain-funcs python-any-r1 multiprocessing
...
set_build_args() {
	...
	# [Mod] Ozone platform configurations are removed.
...
sandboxless_ensure_directory() {
	...
	for dir in "$@"; do
		if [[ ! -d "${dir}" ]] ; then
			...
			sudo chown "${PORTAGE_USERNAME}:${PORTAGE_GRPNAME}" "${dir}" || die "Cannot change owner of ${dir}."
			sudo chmod 0755 "${dir}" || die "Cannot change attributes of ${dir}."
		fi
	done
}
...
src_unpack() {
	...
	*)
	...
	(SERVER_SOURCE)
		...
		if [[ ${WHOAMI} == "chrome-bot" ]]; then
			if [[ -d "${CHROME_DISTDIR}/src" ]]; then
				# TODO: Should add a sanity check that the version checked out
				# is what we actually want.  Not sure how to do that though.
	...
	# [Mod] Use flags afdo_use, afdo_verify, orderfile_verify and orderfile_use
	# are all disabled.
...
# [Mod] src_prepare() is simplified because 1) we do not need to access Google
...
src_configure() {
	...
	if [[ "${CHROME_ORIGIN}" == "SERVER_SOURCE" ]]; then
		...
		"${cmd[@]}" || die "Failed running gclient runhooks."
	fi
...
chrome_make() {
	...
	export PATH=${PATH}:${DEPOT_TOOLS}
	"${command[@]}" || die "Failed ninja build."
}
...
# [Mod] src_install() is greatly simplified and totally new.
src_install() {
	...
	rsync -rpm --chmod=D755,F644 --mkpath --delete     \
		...
		"${icu_header_dir}" || die "Failed rsync of icu headers."
}

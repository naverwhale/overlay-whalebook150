...
RDEPEND="!<=sys-kernel/linux-firmware-0.0.1-r772"
...
LICENSE="
	...
	builtin_fw_intel_apl_i915? ( LICENSE.i915 )
	builtin_fw_intel_jsl_i915? ( LICENSE.i915 )
	builtin_fw_intel_kbl_i915? ( LICENSE.i915 )
...
install_intel_firmwares() {
	if use_builtin_fw intel_apl_i915; then
		doins_subdir i915/bxt_dmc_ver1_07.bin
	fi

	if use_builtin_fw intel_mtl_i915; then
		doins_subdir i915/mtl_dmc.bin
		doins_subdir i915/mtl_guc_70.bin
		doins_subdir i915/mtl_huc_gsc.bin
	fi

	if use_builtin_fw intel_jsl_i915; then
		doins_subdir i915/ehl*
		doins_subdir i915/icl_dmc_ver1_09.bin
	fi

	if use_builtin_fw intel_kbl_i915; then
		doins_subdir i915/kbl*
	fi
}

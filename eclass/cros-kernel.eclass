...
# When you change this eclass, please update
# sys-kernel/chromeos-kernel-*/*/files/revision_bump files to cause package
# rebuilds, working around https://issuetracker.google.com/201299127.
#
# The content of revision_bump files does not matter, as long as it changes.
...
# @ECLASS-VARIABLE: AFDO_PROFILE_VERSION
# @DEFAULT_UNSET
# @DESCRIPTION:
# AFDO profile version to use for X86. Autorolled by pupr.

# @ECLASS-VARIABLE: ARM_AFDO_PROFILE_VERSION
# @DEFAULT_UNSET
# @DESCRIPTION:
# AFDO profile version to use for ARM. Autorolled by pupr.

# @ECLASS-VARIABLE: AFDO_FROZEN_PROFILE_VERSION
# @DEFAULT_UNSET
# @DESCRIPTION:
# Frozen X86 AFDO profile version to use. If set, overrides the autorolled
# AFDO_PROFILE_VERSION variable.

# @ECLASS-VARIABLE: ARM_AFDO_FROZEN_PROFILE_VERSION
# @DEFAULT_UNSET
# @DESCRIPTION:
# Frozen ARM AFDO profile version to use. If set, overrides the autorolled
# ARM_AFDO_PROFILE_VERSION variable.

# @ECLASS-VARIABLE: AFDO_FORCE_DISABLE
# @DEFAULT_UNSET
# @DESCRIPTION:
# `USE=kernel_afdo` requires profile information, but some ebuilds (e.g.,
# kdump) intentionally don't provide that. This makes using `emerge` with
# `USE=kernel_afdo` cumbersome.
#
# We could let the lack of profile information _implicitly_ cause
# `USE=kernel_afdo` to be ignored, but it seems better to require a marker
# affirming that AFDO should be ignored for a package.
#
# Set this to a nonempty value to forcibly ignore AFDO settings.
...
# happen.
inherit binutils-funcs cros-board cros-constants cros-kernel-versions estack cros-toolchain-funcs
...
RDEPEND="
	debug? ( !chromeos-base/tast-use-flags[shipping_kernel] )
"

DEPEND="
	${RDEPEND}
	sys-kernel/linux-firmware:=
	sys-kernel/linux-firmware-builtin:=
	...
	flexor_ramfs? ( chromeos-base/chromeos-initramfs[flexor_ramfs] )
	minios_ramfs? ( chromeos-base/chromeos-initramfs[minios_ramfs] )
	recovery_ramfs? ( chromeos-base/chromeos-initramfs[recovery_ramfs] )
	desktop_recovery_ramfs? ( chromeos-base/chromeos-initramfs[desktop_recovery_ramfs] )
	prod_ramfs? ( chromeos-base/chromeos-initramfs[prod_ramfs] )
	...
	builtin_fw_x86_adl_ucode? (
		...
		!builtin_fw_x86_rpl_ucode? ( sys-firmware/adl-ucode-firmware-private )
	)
	builtin_fw_x86_adln_ucode? ( sys-firmware/adln-ucode-firmware-private )
	builtin_fw_x86_amd_ucode? ( || (
		sys-kernel/linux-firmware[linux_firmware_amd_ucode]
		chromeos-base/amd-ucode-firmware-private
	) )
	...
	builtin_fw_x86_czn_ucode? ( sys-firmware/czn-ucode-firmware-private )
	...
	builtin_fw_x86_pco_ucode? ( sys-firmware/pco-ucode-firmware-private )
	builtin_fw_x86_rpl_ucode? (
		builtin_fw_x86_adl_ucode? ( sys-firmware/brya-kernel-ucode )
		!builtin_fw_x86_adl_ucode? ( sys-firmware/rpl-ucode-firmware-private )
	)
	...
	device_tree_overlay? ( chromeos-base/chromeos-config:= )
"
...
REQUIRED_USE="
	...
	device_tree_overlay? ( device_tree )
...
if [[ -z "${AFDO_FORCE_DISABLE:-}" ]]; then
	...
	REQUIRED_USE+="
		kernel_afdo? ( clang )
		kernel_afdo_verify? ( kernel_afdo )
	"
fi
...
if [[ -z "${AFDO_FORCE_DISABLE:-}" ]]; then
	# Legacy: some builders depend on x86 profile names having no arch prefix (e.g.,
	# instead of `X86_AFDO_FROZEN_PROFILE_VERSION`, they're spelled
	# `AFDO_FROZEN_PROFILE_VERSION`). They're renamed here.
	#
	# Prefer the frozen version of afdo profiles if set.
	X86_AFDO_VERSION=${AFDO_FROZEN_PROFILE_VERSION:-${AFDO_PROFILE_VERSION}}
	...
	if [[ -n "${X86_AFDO_VERSION}" ]]; then
		: "${AFDO_LOCATION:="${GS_PREFIX}/amd64/${KERNEL_VERSION}"}"
		: "${X86_AFDO_LOCATION:="${AFDO_LOCATION}"}"
		X86_AFDO_PROFILE="${PN}-${X86_AFDO_VERSION}.afdo"
		X86_AFDO_SRC_URI="
			${X86_AFDO_LOCATION}/${X86_AFDO_VERSION}.afdo.xz -> ${X86_AFDO_PROFILE}.xz
		"
	fi
	if [[ -n "${ARM_AFDO_VERSION}" ]]; then
		: "${ARM_AFDO_LOCATION:="${GS_PREFIX}/arm/${KERNEL_VERSION}"}"
		ARM_AFDO_PROFILE="${PN}-${ARM_AFDO_VERSION}.afdo"
		ARM_AFDO_SRC_URI="
			${ARM_AFDO_LOCATION}/${ARM_AFDO_VERSION}.afdo.xz -> ${ARM_AFDO_PROFILE}.xz
		"
	fi

	if [[ -n "${X86_AFDO_VERSION}" && -n "${ARM_AFDO_VERSION}" ]]; then
		SRC_URI+=" kernel_afdo? (
			arm? ( ${ARM_AFDO_SRC_URI} )
			arm64? ( ${ARM_AFDO_SRC_URI} )
			!arm? ( !arm64? ( ${X86_AFDO_SRC_URI} ) )
		) "
	elif [[ -n "${X86_AFDO_VERSION}" ]]; then
		SRC_URI+=" kernel_afdo? ( ${X86_AFDO_SRC_URI} ) "
	elif [[ -n "${ARM_AFDO_VERSION}" ]]; then
		SRC_URI+=" kernel_afdo? ( ${ARM_AFDO_SRC_URI} ) "
	else
		# If no profiles are available, AFDO isn't usable.
		REQUIRED_USE+=" !kernel_afdo"
	fi
fi # [[ -z "${AFDO_FORCE_DISABLE:-}" ]]

_cros-kernel_use_afdo() {
	[[ -z "${AFDO_FORCE_DISABLE:-}" ]] && use kernel_afdo
}

_cros-kernel_use_afdo_verify() {
	[[ -z "${AFDO_FORCE_DISABLE:-}" ]] && use kernel_afdo_verify
}
...
cros-kernel_apply_patches() {
	cros-kernel_have_patches && eapply "${FILESDIR}"/*.patch
}
...
# Downstream fragment that enables the CH7322 HDMI CEC driver. Although
# the driver itself is upstreamed, it is kept disabled by default for
# most ChromeOS platforms and explicitly enabled here to support the
# required Lattice HDMI ingest device used in CfM Series 1 products.
# See b/267878717 for downstream context and platform requirements.
cec_ch7322_desc="CH7322 HDMI CEC support"
cec_ch7322_config="
CONFIG_CEC_CH7322=m
"
...
disable_itco_desc="Disable ITCO watchdog support"
disable_itco_config="
...
earlyprintk_xdbc_desc="USB3 debug port earlyprintk support"
earlyprintk_xdbc_config="
...
# EFI_EARLYCON depends on but doesn't select SERIAL_EARLYCON. Include
# SERIAL_8250_CONSOLE and its dependency SERIAL_8250 to select SERIAL_EARLYCON.
efi_earlycon_desc="Early EFI console support"
efi_earlycon_config="
...
"
# CONFIG_FB_EFI requires both vtconsole and fbconsole.
REQUIRED_USE+=" efi_earlycon? ( vtconsole fbconsole ) "
...
kasan_outline_desc="Enable KASAN (OUTLINE instrumentation)"
kasan_outline_config="
CONFIG_KASAN=y
CONFIG_KASAN_OUTLINE=y
CONFIG_TEST_KASAN=m
CONFIG_FRAME_WARN=0
CONFIG_FORTIFY_SOURCE=n
CONFIG_LOCALVERSION=\"-kasan_outline\"
CONFIG_DEBUG_INFO_BTF=n
# CONFIG_KFENCE is not set
"

kcfi_desc="Enable kcfi (for 6.1 and later)"
kcfi_config="
CONFIG_CFI_CLANG=y
"

gcov_desc="Enable gcov"
gcov_config="
...
# HPT (High Performance Tracing) (go/cros-hpt-g3doc) requires
# GCOV support plus converting GCOV notes into coverage data.
hpt_desc="Enable GCOV for HPT"
hpt_config="
${gcov_config}
"
...
kprobes_desc="Kprobes and kprobe events support"
kprobes_config="
CONFIG_KPROBES=y
CONFIG_KPROBE_EVENTS=y
"

kunit_desc="Enable kunit tests"
kunit_config="
CONFIG_KUNIT=m
...
# initramfs compression is done by the chromeos-initramfs package build
# and kernel build can ingest the compressed image as it is.
desktop_recovery_ramfs_desc="Initramfs for desktop recovery image"
desktop_recovery_ramfs_config='
CONFIG_INITRAMFS_SOURCE="%ROOT%/var/lib/initramfs/desktop_recovery_ramfs.cpio"
CONFIG_INITRAMFS_COMPRESSION_NONE=y
'
...
flexor_ramfs_desc="Initramfs for a flexor image"
flexor_ramfs_config='
CONFIG_INITRAMFS_SOURCE="%ROOT%/var/lib/initramfs/flexor_ramfs.cpio"
CONFIG_INITRAMFS_COMPRESSION_NONE=y
'
...
prod_ramfs_desc="Initramfs for prod Chromebooks"
prod_ramfs_config='
CONFIG_INITRAMFS_SOURCE="%ROOT%/var/lib/initramfs/prod_ramfs.cpio"
CONFIG_INITRAMFS_COMPRESSION_NONE=y
CONFIG_DM_INIT=n
'
...
# This fragment isn't exposed as a USE flag because it is only used to disable
# module compression when this eclass detects that loadpin is enabled without
# module decompression in the kernel.
module_compress_config_disable="
...
usb_gadget_uvc_desc="USB UVC gadget support"
usb_gadget_uvc_config="
CONFIG_USB_CONFIGFS_F_UVC=y
"

# Downstream fragment enabling the Media-Agnostic USB (MA-USB) transport
# for the functionality of CfM Series 1 IP-based touch devices.
# See b/262748749 for background and discussion on this downstream requirement.
usb_host_mausb_desc="MA-USB Host Device Transport Support"
usb_host_mausb_config="
CONFIG_USB_HOST_MAUSB=m
"
...
usb_serial_console_desc="USB serial console support"
usb_serial_console_config="
CONFIG_USB_SERIAL=y
...
waydroid_desc="Configs required by the Waydroid (Android-in-LXC) container.
Enables the binder filesystem (Android T+ uses it instead of static
/dev/binder), memory cgroups (used by powerd's suspend freezer to pause
the container during suspend), and the netfilter NAT/masquerade plus
nf_tables stack waydroid-net.sh expects when bringing up the waydroid0
bridge for container-side networking."
waydroid_config="
...
minios_kernel_desc="Configs for minios"
minios_kernel_config="
CONFIG_DRM_I915=y
"

lenovo300e_kernel_desc="Configs for Whalebook LENOVO300E"
lenovo300e_kernel_config="
CONFIG_WHALEBOOK=y
CONFIG_WHALEBOOK_LENOVO300E=y
...
CONFIG_WLAN_VENDOR_INTEL=m
CONFIG_IWLWIFI_DEBUG=m
CONFIG_IWLWIFI_DEBUG_FS=m
...
lg14u390_kernel_desc="Configs for Whalebook LG14U390"
lg14u390_kernel_config="
CONFIG_WHALEBOOK=y
CONFIG_WHALEBOOK_LG14U390=y
...
CONFIG_TCG_CRB=y
CONFIG_EFIVAR_FS=y
CONFIG_LEDS_TRIGGER_SDHCI=y
CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=n
CONFIG_SECURITY_LOCKDOWN_LSM=n
CONFIG_SECURITY_LOCKDOWN_LSM_EARLY=n
# LG14U390 is the only board whose vmlinuz is signed externally: we build a
# nosign image, hand the vmlinuz to the signing vendor, drop the signed blob
# into overlay-lg14u390/files/vmlinuz.bin and then swap it in during the sign
# build (see overlay-lg14u390/scripts/board_specific_setup.sh).  The kernel that
# ends up booting is therefore from the *nosign* build while the modules come
# from the sign build.  Any relink of vmlinux between the two builds shifts the
# vmlinux BTF type IDs, so the modules' split BTF no longer resolves and every
# module is rejected with -EINVAL, leaving the machine without graphics, input
# or networking.  BTF is only consumed by BPF tooling and plays no part in
# loading or running a module, so tolerate the mismatch instead of refusing the
# module.  The minios_ramfs and factory_netboot_ramfs fragments above do the
# same thing for the same reason.
CONFIG_MODULE_ALLOW_BTF_MISMATCH=y
"

whaleos_amd64_kernel_desc="Configs for WhaleOS Flex"
whaleos_amd64_kernel_config="
CONFIG_WHALEBOOK=y
CONFIG_WHALEOS_AMD64=y
...
FIRMWARE_BINARIES=(
	...
	builtin_fw_huc_adln
	builtin_fw_huc_tgl
	builtin_fw_intel_apl_i915
	builtin_fw_intel_kbl_i915
	builtin_fw_intel_jsl_i915
	builtin_fw_intel_mtl_i915
	builtin_fw_mali_g57
...
builtin_fw_amdgpu_gc_11_0_4_desc="Firmware for AMD GC 11.0.4"
builtin_fw_amdgpu_gc_11_0_4_files=(
...
builtin_fw_intel_apl_i915_desc="DMC i915 Firmware for APL"
builtin_fw_intel_apl_i915_files=(
	i915/bxt_dmc_ver1_07.bin
)

builtin_fw_intel_kbl_i915_desc="GuC, HuC, DMC i915 Firmware for KBL"
builtin_fw_intel_kbl_i915_files=(
	i915/kbl_guc_ver9_39.bin
	i915/kbl_huc_ver02_00_1810.bin
	i915/kbl_dmc_ver1_04.bin
)

builtin_fw_intel_jsl_i915_desc="GuC, HuC, DMC i915 Firmware for JSL"
builtin_fw_intel_jsl_i915_files=(
...
builtin_fw_dmc_tgl_desc="DMC i915 Firmware for TGL"
builtin_fw_dmc_tgl_files=(
...
builtin_fw_intel_mtl_i915_desc="GuC, HuC, DMC i915 Firmwares for MTL"
builtin_fw_intel_mtl_i915_files=(
	i915/mtl_guc_70.bin
...
builtin_fw_x86_intel_ucode_files=(
	...
	intel-ucode/06-8f-07
	intel-ucode/06-8f-08
	...
	intel-ucode/06-aa-04
	intel-ucode/06-ad-01
	intel-ucode/06-af-03
	intel-ucode/06-b5-00
	intel-ucode/06-b7-01
	intel-ucode/06-ba-02
	intel-ucode/06-ba-03
	intel-ucode/06-bd-01
	intel-ucode/06-be-00
	intel-ucode/06-bf-02
	intel-ucode/06-bf-05
	intel-ucode/06-c5-02
	intel-ucode/06-c6-02
	intel-ucode/06-cf-02
...
builtin_fw_x86_czn_ucode_desc="AMD ucode for Cezanne"
builtin_fw_x86_czn_ucode_files=(
	amd-ucode/microcode_amd_fam19h.bin
)
...
builtin_fw_x86_pco_ucode_desc="AMD ucode for Picasso"
builtin_fw_x86_pco_ucode_files=(
	amd-ucode/microcode_amd_fam17h.bin
)
...
# @FUNCTION: _cros-kernel_get_fit_compression
# @USAGE:
# @DESCRIPTION:
...
_cros-kernel_get_fit_compression() {
...
# @FUNCTION: _cros-kernel_get_fit_compressed_kernel_path
# @USAGE:
# @DESCRIPTION:
...
_cros-kernel_get_compressed_path() {
	local uncompressed_path="$(_cros-kernel_get_fit_kernel_path)"
	local compression="$(_cros-kernel_get_fit_compression)"
...
get_compression_level_arg() {
	local compression="$1"

	case "${compression}" in
		lz4)
			# Best lz4 compression level.
			echo -12
			;;
		lzma)
			# Best lzma compression level.
			echo -9
			;;
	esac
}

# @FUNCTION: _cros-kernel_compress_fit_kernel
# @USAGE: <kernel_dir>
# @DESCRIPTION:
# Compresses the kernel with the algorithm selected by current USE flags. If
# no compression algorithm this does nothing.
_cros-kernel_compress_fit_kernel() {
	local kernel_dir=${1}
	local kernel_path="${kernel_dir}/$(_cros-kernel_get_fit_kernel_path)"
	local compr_path="${kernel_dir}/$(_cros-kernel_get_compressed_path)"
	local compression="$(_cros-kernel_get_fit_compression)"
	local compr_level_arg="$(get_compression_level_arg "${compression}")"

	case "${compression}" in
		lz4)
			lz4 "${compr_level_arg}" -z -f "${kernel_path}" "${compr_path}" || die
			;;
		lzma)
			lzma "${compr_level_arg}" -z -f -k "${kernel_path}" || die
			;;
	esac
}

# @FUNCTION: _cros-kernel_get_compat
# @USAGE: <dtb file>
# @DESCRIPTION:
...
# @FUNCTION: _cros-kernel_emit_its_script
...
_cros-kernel_emit_its_script() {
	...
	fit_compression_kernel="$(_cros-kernel_get_fit_compression)"
	kernel_bin_path="${kernel_dir}/$(_cros-kernel_get_compressed_path)"
	kernel_os="$(usex kernel-efi "efi" "linux")"
...
kmake() {
	...
	_cros-kernel_use_afdo && kcflags+=" -fprofile-sample-use=${WORKDIR}/${AFDO_PROFILE}"
	...
	if use clang; then
		# LLVM needs this to parse perf.data. See AutoFDO README for
		# details: https://github.com/google/autofdo
		kcflags+=" -fdebug-info-for-profiling "

		# TODO(b/307741717): A newer version of Clang flags 16 or so
		# instances of this.across at least 4 kernel versions (6.1,
		# 5.15, 5.10, and 5.4). That should be cleaned up.
		kcflags+=" -Wno-error=fortify-source "

		# b/444153195
		# Some code triggers -Wdefault-const-init-var-unsafe.
		# The diagnostic has been turned off in upstream:
		# https://lkml.org/lkml/2025/5/11/45. Turn it off here, too.
		# Like upstream, we use -Wno-default-const-init-unsafe,
		# which also turns off -Wdefault-const-init-field-unsafe.
		#
		# Older Clang versions do not support these flags, so first check
		# if they are supported and only pass them if so.
		if $(tc-getCC) -fsyntax-only -x c /dev/null -Werror=unknown-warning-option \
			-Wno-default-const-init-unsafe 2>/dev/null; then
			kcflags+=" -Wno-default-const-init-unsafe "
		fi

		# b/444035882: Do not synthesize calls to wcslen.
		kcflags+=" -fno-builtin-wcslen "
	fi
	...
	# Per Documentation/kbuild/reproducible-builds.rst, provide
	# KBUILD_BUILD_TIMESTAMP from source timestamp instead of the current
	# time.
	source_timestamp="$(git log -1 --format=%cD 2>/dev/null || echo 0)"

	ARCH=${kernel_arch} \
		CROSS_COMPILE="${cross}-" \
		CROSS_COMPILE_COMPAT="${cross_compat}" \
		KCFLAGS="${kcflags}" \
		KBUILD_BUILD_HOST=chromium.org \
		KBUILD_BUILD_USER=cros-kernel \
		KBUILD_BUILD_TIMESTAMP="${source_timestamp}" \
...
# Set a few globals describing the AFDO profile to use. Must be called exactly
# once if using AFDO in a build, with an argument of either "x86" or "arm".
_cros-kernel_set_afdo_profile() {
	local ty="$1"
	case "${ty}" in
		x86 )
			einfo "Using X86 AFDO profile for this build."
			AFDO_VERSION="${X86_AFDO_VERSION}"
			AFDO_PROFILE="${X86_AFDO_PROFILE}"
			;;
		arm )
			einfo "Using ARM AFDO profile for this build."
			AFDO_VERSION="${ARM_AFDO_VERSION}"
			AFDO_PROFILE="${ARM_AFDO_PROFILE}"
			;;

		* )
			die "Unknown AFDO profile type: ${ty}"
	esac
}

cros-kernel_src_unpack() {
	# Force in-tree builds if private patches may have to be applied.
	if cros-kernel_have_patches; then
		CROS_WORKON_OUTOFTREE_BUILD=0
	fi

	cros-workon_src_unpack
	if _cros-kernel_use_afdo; then
		if use arm || use arm64; then
			if [[ -n "${ARM_AFDO_VERSION}" ]]; then
				_cros-kernel_set_afdo_profile arm
			else
				_cros-kernel_set_afdo_profile x86
			fi
		else
			if [[ -n "${X86_AFDO_VERSION}" ]]; then
				_cros-kernel_set_afdo_profile x86
...
cros-kernel_src_configure() {
	...
	local build_salt="CONFIG_BUILD_SALT=\"${PF}\""
	...
	if ! use disable_kernel_fragments; then
		...
		# Aggregate CONFIG_LOCALVERSION as it is possible provided multiple times.
		# The last assignment is what is used.  The latter one overrides previous
		# ones during kernel config normalization.  See also
		# https://elixir.bootlin.com/linux/v6.14/source/scripts/kconfig/confdata.c#L403
		if grep -q CONFIG_LOCALVERSION= "${build_cfg}"; then
			local localversion=$(
				sed -n -E '/^CONFIG_LOCALVERSION=/{s:^[^=]*="(.*)":\1:p}' \
				"${build_cfg}" | sort -u | tr -d '\n')
			echo "CONFIG_LOCALVERSION=\"${localversion}\"" >> "${build_cfg}" || die
		fi
	fi
	...
	if [[ -e "${old_config}" && -e "${old_defconfig}" && \
		...
		# Remove module compression if we're enforcing loadpin but the
		# kernel doesn't support module decompression in the kernel
		# because loadpin will block loading compressed modules. See
		# kernel commit b1ae6dc41eaa ("module: add in-kernel support for
		# decompressing") for more details.
		if ! cros_chkconfig_present MODULE_DECOMPRESS &&
			cros_chkconfig_present SECURITY_LOADPIN_ENFORCE &&
			cros_chkconfig_present "MODULE_COMPRESS_\(GZIP\|XZ\|ZSTD\)" ; then

			ewarn "Removing module compression due to missing in-kernel module decompression (CONFIG_MODULE_DECOMPRESS)"

			echo "${module_compress_config_disable}" >> "${build_cfg}" || die
...
gen_compilation_database() {
	...
	# shellcheck disable=SC2154
	local ext_chroot_path="${EXTERNAL_TRUNK_PATH}/out"
...
_cros-kernel_compile() {
	...
	# If a .dts(o) file is deleted from the source code it won't disappear
	...
	[[ -d "${arch_dir}" ]] && \
		find "${arch_dir}" \( -name '*.dtb' -o -name '*.dtbo' \) -delete
...
cros-kernel_src_compile() {
	...
	# Some users of cros-kernel touch the config after
	# cros-kernel_src_configure finishes.  Detect that and remove
	# the old configs we were saving to speed up the next
	# incremental build.  These users of cros-kernel will be
	...
	# cros-kernel_src_configure() if we see that our config changed (we
...
cros-kernel_src_test() {
	local script_dir="${S}/chromeos/scripts"
	if [[ -f "${script_dir}/Makefile" ]]; then
		make -C "${script_dir}" test || die "test failed"
	fi
}

# @FUNCTION: cros-kernel_src_install
...
cros-kernel_src_install() {
	...
	if use device_tree && ! use device_tree_overlay; then
		_cros-kernel_compress_fit_kernel \
	...
	if use arm || use arm64 || use mips; then
		...
		local dtb_dir="${boot_dir}/dts"

		if use device_tree; then
			if use device_tree_overlay; then
				local dtb_config="${CHROMEOS_DTB_CONFIG:-}"
				if [[ -z "${dtb_config}" ]]; then
					die "CHROMEOS_DTB_CONFIG must be defined"
				fi
				local script_dir="${S}/chromeos/scripts"
				local compression="$(_cros-kernel_get_fit_compression)"
				local compression_arg="$(get_compression_level_arg "${compression}")"
				local kernel_os="$(usex kernel-efi "efi" "linux")"
				local kernel_bin_path="${kernel_dir}/$(_cros-kernel_get_fit_kernel_path)"
				local cros_config_path="${SYSROOT}/usr/share/chromeos-config/yaml/config.yaml"
				local dtb_config_path="${S}/arch/${kernel_arch}/boot/dts/${dtb_config}"
				local dtb_sub_dir="${dtb_dir}/$(dirname "${dtb_config}")"
				(cd "${dtb_sub_dir}" && "${script_dir}/cros_make_fit.py" \
					-c "${compression} ${compression_arg}" \
					-n "ChromeOS kernel image" \
					-A "${kernel_arch}" \
					-O "${kernel_os}" \
					-k "${kernel_bin_path}" \
					--chromeos-config "${cros_config_path}" \
					--dtb-config "${dtb_config_path}" \
					-v \
					-o "${kernel_bin}") || die
			else
				local its_script="${kernel_dir}/its_script"
				# get_dtb_name() produces a line-separated list, and we *want* to
				# split on whitespace for _cros-kernel_emit_its_script(). These
				# are simple filenames, so this should be OK.
				# shellcheck disable=SC2046
				_cros-kernel_emit_its_script "${its_script}" \
...
directory (outside of the chroot) to compile_commands_no_chroot.json."
	...
	if _cros-kernel_use_afdo_verify; then
		# Deliver the profile we just verified.  The upload artifacts
		# step in the builder will collect the (now verified) AFDO
		# profile from here.
		insinto "${install_prefix}/usr/lib/debug/boot"
		doins "${DISTDIR}/${AFDO_PROFILE}.xz"
	fi

	# Pack the GCOV notes needed for coverage analysis.
	if use gcov; then
		local kernel_src="$(cros-workon_get_build_dir)"

		# Deliver the GCOV notes (gcno) file. The upload artifacts
		# step in the builder will collect this file from here.
		local gcov_file="${kernel_src}/gcov_gcno.txz"
		find "${kernel_src}" -name '*.gcno' |\
		tar --create --xz --file "${gcov_file}" -C "${kernel_src}" -v -T -
		einfo "Packed GCOV coverage notes for ${PN}"

		insinto "${install_prefix}/usr/lib/debug/boot"
		doins "${gcov_file}"
	fi

	# Generate the coverage artifacts needed for kernel instrumentation.
	if use hpt; then
		local kernel_src="$(cros-workon_get_build_dir)"
		local hpt_cov_dir="${CROS_ARTIFACTS_TMP_DIR:?}/hpt_coverage"

		mkdir -p "${hpt_cov_dir}" || die

		"${CHROMITE_DIR}/scripts/cros_generate_coverage_artifacts" \
			--kernel-home "${kernel_src}" \
			--output-dir "${hpt_cov_dir}" || die
		einfo "E2E coverage report for ${PN} generated at ${hpt_cov_dir}"
	fi
}
